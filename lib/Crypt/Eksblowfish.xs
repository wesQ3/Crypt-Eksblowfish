...
typedef BF_word BF_round_key_set[BF_N + 2];
 ...
 */
static const BF_key_schedule BF_init_state = {
	{
...
#define BF_ROUND(L, R, N) \
	...
	tmp4 = L >> 22; \
	tmp4 &= 0x3FC; \
	tmp1 = BF_INDEX(ks->S[3], tmp1); \
	tmp2 = BF_INDEX(ks->S[2], tmp2); \
	tmp3 = BF_INDEX(ks->S[1], tmp3); \
	tmp3 += BF_INDEX(ks->S[0], tmp4); \
	tmp3 ^= tmp2; \
	R ^= ks->P[N + 1]; \
	tmp3 += tmp1; \
	R ^= tmp3;
...
static BF_block encrypt_block(const BF_key_schedule *ks, BF_block blk)
{
	BF_word tmp1, tmp2, tmp3, tmp4, t;
	blk.L ^= ks->P[0];
	BF_ROUND(blk.L, blk.R, 0);
	BF_ROUND(blk.R, blk.L, 1);
	BF_ROUND(blk.L, blk.R, 2);
	BF_ROUND(blk.R, blk.L, 3);
	BF_ROUND(blk.L, blk.R, 4);
	BF_ROUND(blk.R, blk.L, 5);
	BF_ROUND(blk.L, blk.R, 6);
	BF_ROUND(blk.R, blk.L, 7);
	BF_ROUND(blk.L, blk.R, 8);
	BF_ROUND(blk.R, blk.L, 9);
	BF_ROUND(blk.L, blk.R, 10);
	BF_ROUND(blk.R, blk.L, 11);
	BF_ROUND(blk.L, blk.R, 12);
	BF_ROUND(blk.R, blk.L, 13);
	BF_ROUND(blk.L, blk.R, 14);
	BF_ROUND(blk.R, blk.L, 15);
	t = blk.R;
	blk.R = blk.L;
	blk.L = t ^ ks->P[BF_N + 1];
	return blk;
...
/*
 * Decrypt one block, BF_N is hardcoded here.
 */

static BF_block decrypt_block(const BF_key_schedule *ks, BF_block blk)
{
	BF_word tmp1, tmp2, tmp3, tmp4, t;
	t = blk.L ^ ks->P[BF_N + 1];
	blk.L = blk.R;
	...
	const unsigned char *key, size_t keylen, BF_key_schedule *ks)
{
	...
	for(i = 0; i != 4; i += 2) {
		...
		expanded_salt[i] = sblk.L;
		expanded_salt[i+1] = sblk.R;
	}
	for(; i != BF_N + 2; i++) {
		expanded_salt[i] = expanded_salt[i & 3];
	}
